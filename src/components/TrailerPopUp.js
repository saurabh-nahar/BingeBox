...
const TrailerPopUp = ({ title, id, desc, onClose }) => {
  ...
  return (
    <div className="fixed top-0 left-0 z-50 w-full h-full bg-black bg-opacity-90 flex flex-col justify-center items-center overflow-hidden">
      <div className="relative z-50 w-full md:w-3/4 h-[80vh] bg-black text-white p-8 rounded-lg overflow-hidden">
        <button
          className="absolute z-50 top-4 p-4 right-4 text-2xl"
          onClick={onClose}
        >
             ×  
        </button>
        ...
        <div className="w-full h-full flex justify-center">
          <iframe
            className="w-full md:w-3/4 h-3/4 md:h-3/4 "
